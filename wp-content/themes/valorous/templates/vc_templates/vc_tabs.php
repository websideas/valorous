<?php
$output = $title = $interval = $el_class = $icon_type = $tabs_color = $tabs_positions = '';
...
foreach ( $tab_titles as $tab ) {
    ...
    vc_icon_element_fonts_enqueue( $tab_atts['icon_type'] );
    if($tab_atts['icon_type']){
        if( isset( $tab_atts['icon_'.$tab_atts['icon_type']] ) ){
            $icon = '<span class="vc_icon_element-icon '.$tab_atts['icon_'. $tab_atts['icon_type']].'"></span>';
        }else{
            $icon = '';
        }
    }
